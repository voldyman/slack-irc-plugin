...
Bot.prototype._trackUsers = function () {
    ...
    this.client.addListener('join', function (channel, nick, whois) {
        if (whois.user == myusername) {
            ...
            self._usermap.nicks[nick] = self._usermap.users[whois.user];
            self.whisper(nick, 'i\'m watching you slacker @' +
                       self._usermap.nicks[nick]);

            if (self.config.giveOps) {
...
/**
 * Push a notice to a channel
 * @param {string} target IRC channel/user name
 * @param {string} message Message to push to target
 */
Bot.prototype.whisper = function (target, message) {
